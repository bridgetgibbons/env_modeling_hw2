...
### standard deviation is 0.20 * mean
...
q.arg = list(list(mean = -0.015, sd = sd_x1), list(mean = -0.0046, sd = sd_x2), list(mean = -0.07, sd = sd_x3), list(mean = 0.0043, sd = sd_x4), )
...
LHS_results = matrix(nrow=nsets, ncol= 7)

# Now running the model over these parameter sets
### We will use the faster way, using mapply (versus for loop)

LHS_model_results = mapply(FUN=yield_function_assignment3, x1=LHS_pars$x1, x2=LHS_pars$x2, x3=LHS_pars$x3, x4=LHS_pars$x4, intercept=LHS_pars$intercept, climate=LHS_pars$climate,  MoreArgs=list(clim=clim))

# Putting the results in our previously created matrix

LHS_results = matrix((unlist(LHS_model_results)), ncol= 7, byrow=TRUE)

### Assigning column names

colnames(LHS_results)=c("x1","x2", "x3", "x4", )
...
y = pse::tell(y, t(LHS_results), res.names=c("x1","x2", "x3", "x4"))
...
ggplot(LHS_results, aes(x1, x2, x3, x4))+geom_point()+labs(y="x1", "x2", "x3", "x4")
...
sobel_result = mapply(FUN = yield_function_assignment3, x1 = sens_yield_sobel$X$sobel_x1, x2 = sens_yield_sobel$X$sobel_x2, x3 = sens_yield_sobel$X$sobel_x3, x4 = sens_yield_sobel$X$sobel_x4, MoreArgs = list(climate = clim))
